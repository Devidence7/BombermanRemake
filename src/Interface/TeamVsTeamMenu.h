...
public:
	TeamVsTeamMenu(sf::RenderWindow& window) {
	...
	void actualizeNumPLayers(int numPlayers) {
...
public:
	void menuActions(GameDisplayController& gameDisplay, Game game) {
		...
		gameDisplay.manageGameInterface(gameDisplay, std::bind(&TeamVsTeamMenu::userActions, this, std::placeholders::_1, std::ref(gameDisplay.getWindow()), std::ref(gameDisplay), std::ref(game)));
