...
namespace GameGUI {
	...
	Widget* Layout::searchFinalFocussedWidget() {
		if (m_focus == NULL) {
			return nullptr;
		}
		else {
			if (m_focus->editedWidget) {
				return m_focus;
			}
			else {
				return m_focus->searchFinalFocussedWidget();
			}
		}
	}
	...
	void Layout::onKeyPressed(sf::Keyboard::Key key) {
		if (m_focus == NULL) {
			...
			else if (key == Theme::nextWidgetKey || key == sf::Keyboard::Key::S || (key == sf::Keyboard::Key::Tab && !sf::Keyboard::isKeyPressed(sf::Keyboard::Key::LShift))) {
				if (!focusNextWidget())
					focusNextWidget();
			}
